"""
Testes do filtro de Processo por Cliente nos formulários do financeiro
e do endpoint que o alimenta (specs/filtro-processo-por-cliente.md).

Segue o mesmo padrão de fixtures de apps/financeiro/tests/test_honorarios.py
...
class FiltroProcessoClienteBase(TenantTestCase):
    def setUp(self):
        super().setUp()
...
class TestLancamentoFinanceiroFormValidaProcessoDoCliente(FiltroProcessoClienteBase):
    ...
    def get_test_schema_name(cls):
        return "financeiro_filtro_processo_cliente"

    def setUp(self):
        super().setUp()
        self.user = self._user("resp_lancamentos")
    ...
    def test_queryset_do_processo_e_restrito_ao_cliente_ja_selecionado(self):
        form = LancamentoFinanceiroForm(data=self._dados_base(cliente=self.cliente_a.id))
    ...
    def test_processo_de_outro_cliente_e_rejeitado_na_validacao(self):
        form = LancamentoFinanceiroForm(data=self._dados_base(
            cliente=self.cliente_a.id, processo=self.processo_b.id,
        ))
    ...
    def test_processo_do_mesmo_cliente_e_aceito(self):
        form = LancamentoFinanceiroForm(data=self._dados_base(
            cliente=self.cliente_a.id, processo=self.processo_a.id,
        ))
        self.assertTrue(form.is_valid(), form.errors)

    def test_sem_cliente_selecionado_lista_todos_os_processos_ativos(self):
        form = LancamentoFinanceiroForm(data=self._dados_base())
        self.assertIn(self.processo_a, form.fields["processo"].queryset)
        self.assertIn(self.processo_b, form.fields["processo"].queryset)


class TestEndpointProcessosPorCliente(FiltroProcessoClienteBase):
    @classmethod
    def get_test_schema_name(cls):
        return "financeiro_endpoint_processos_cliente"

    def setUp(self):
        super().setUp()
        self.user = self._user("com_acesso_financeiro")
        self._conceder_modulo(self.user)
        self.sem_acesso = self._user("sem_acesso_financeiro")
    ...
    def test_sem_permissao_do_modulo_financeiro_retorna_403(self):
