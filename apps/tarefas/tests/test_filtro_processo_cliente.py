"""
Testes do filtro de Processo por Cliente no formulário de Tarefas e do
endpoint que o alimenta (specs/filtro-processo-por-cliente.md).
...
class FiltroProcessoClienteTarefasBase(TenantTestCase):
    def setUp(self):
        super().setUp()
...
class TestTarefaFormValidaProcessoDoCliente(FiltroProcessoClienteTarefasBase):
    @classmethod
    ...
    def _dados_base(self, **extra):
        dados = {"titulo": "Tarefa Teste", "prioridade": "media"}
        dados.update(extra)
        return dados

    def test_queryset_do_processo_e_restrito_ao_cliente_ja_selecionado(self):
        form = TarefaForm(data=self._dados_base(cliente=self.cliente_a.id))
    ...
    def test_processo_de_outro_cliente_e_rejeitado_na_validacao(self):
        form = TarefaForm(data=self._dados_base(cliente=self.cliente_a.id, processo=self.processo_b.id))
        self.assertFalse(form.is_valid())
        self.assertIn("processo", form.errors)

    def test_processo_do_mesmo_cliente_e_aceito(self):
        form = TarefaForm(data=self._dados_base(cliente=self.cliente_a.id, processo=self.processo_a.id))
        self.assertTrue(form.is_valid(), form.errors)


class TestEndpointProcessosPorClienteTarefas(FiltroProcessoClienteTarefasBase):
    @classmethod
    def get_test_schema_name(cls):
    ...
    def setUp(self):
        super().setUp()
        self.user = self._user("com_acesso_tarefas")
        self._conceder_modulo(self.user)
        self.sem_acesso = self._user("sem_acesso_tarefas")
    ...
    def test_sem_permissao_do_modulo_tarefas_retorna_403(self):
