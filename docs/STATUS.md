...
| Agenda | Parcial (autorização de módulo + escopo por responsável aplicados nas views; lembrete automático 15min antes — PDR-0016; integridade cliente-processo validada no backend e refletida no formulário) | Escopo por participante não existe (campo não exposto em `CompromissoForm`, sem regra de produto) |
