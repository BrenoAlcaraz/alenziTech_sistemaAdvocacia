...
| Agenda | Feito (autorização de módulo + escopo por responsável e por participante aplicados nas views; participantes com confirmação de presença individual — pendente/confirmado/recusado, sem habilitação nova — PDR-0020; lembrete automático 15min antes alcança responsável e participante confirmado — PDR-0016/PDR-0020; compromisso cancelado sai da grade operacional, fica consultável por 7 dias a partir do cancelamento e é expurgado automaticamente — PDR-0020; Dashboard com blocos pessoais de confirmados/pendentes de confirmação; integridade cliente-processo validada no backend e refletida no formulário; filtro por escopo/participante — Agenda e bloco de confirmados do Dashboard — usa `.distinct()` após o `Q(responsavel=...) | Q(participacoes__...)`, para não duplicar compromisso com múltiplos participantes convidados) | Geração automática de prazo processual a partir de andamento ainda não existe — spec preparada em `specs/agenda-prazos-processuais-automaticos.md`, parada aguardando validação jurídica externa do catálogo de prazos |
